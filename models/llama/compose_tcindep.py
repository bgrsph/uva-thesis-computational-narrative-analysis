...
def main(temporal_path: Path, causal_path: Path, output_path: Path) -> int:
    ...
    # Only compose for keys where Llama actually ran on BOTH sides. Skip when either side is source='skipped_ctx_overflow' (or any other non-llama source): 
    #   the merger will surface those overflow rows under their own condition slots. tcindep is just absent for that summary.
    keys = sorted(
